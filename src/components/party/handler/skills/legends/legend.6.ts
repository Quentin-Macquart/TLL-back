...
export class Robin extends LegendSkills {
  ...
  ultimate(correspSummoners: { emittor: Summoner; receptors: Summoner[] }, skill: Skill): void {
    // Check the column of the legend - Detect if there are other summoners -> if yes > this.skillCalculator.dmtToAllReceptor(corr, skilll)
  }
  ...
  public castSkill(correspSummoners: { emittor: Summoner; receptors: Summoner[] }, skill: Skill): void {
    switch (skill.number) {
      case 1:
        // this.skillCalculator.updateAttackWithMissingLifePoints(correspSummoners.emittor, skill);
        break;
      case 2:
